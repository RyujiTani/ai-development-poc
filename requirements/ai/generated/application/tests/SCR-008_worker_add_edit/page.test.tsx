...
const mocks = vi.hoisted(() => {
  ...
  return {
    mockPush,
    mockGet,
    mockExecuteSaveWorker,
    mockExecuteGetWorkerById,
    mockRouter: {
      push: mockPush,
    },
    mockSearchParams: {
...
vi.mock('../../features/worker/usecase/saveWorkerUseCase', () => {
  return {
    SaveWorkerUseCase: vi.fn().mockImplementation(() => {
      return {
        execute: mocks.mockExecuteSaveWorker,
      };
    }),
  };
});
...
vi.mock('../../features/worker/usecase/getWorkerByIdUseCase', () => {
  return {
    GetWorkerByIdUseCase: vi.fn().mockImplementation(() => {
      return {
        execute: mocks.mockExecuteGetWorkerById,
      };
    }),
  };
});
...
describe('SCR-008: 作業員追加・編集画面のテスト', () => {
  ...
  describe('新規追加モード (WorkerAddPage)', () => {
    ...
    it('TS-SCR-008-004: 正常な入力値で保存した際、SaveWorkerUseCaseが呼ばれ、一覧へ遷移すること', async () => {
      ...
      mocks.mockExecuteSaveWorker.mockResolvedValue({
        success: true,
        value: {
          worker_id: 'w123',
      ...
      await waitFor(() => {
        expect(mocks.mockExecuteSaveWorker).toHaveBeenCalledWith({
      ...
      await waitFor(() => {
        expect(mocks.mockPush).toHaveBeenCalledWith('/contractor/workers');
      });
    });
  });
  ...
  describe('編集モード (WorkerEditPage)', () => {
    ...
    it('TS-SCR-008-003: 既存データの読み込みが完了した際、初期値がフォームにロードされていること', async () => {
      ...
      mocks.mockGet.mockReturnValue('w123');
      mocks.mockExecuteGetWorkerById.mockResolvedValue({
        success: true,
        value: {
          worker_id: 'w123',
          contractor_id: 'c1111111',
