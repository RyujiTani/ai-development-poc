...
Python実行環境から提供された3つの入力データを統合し、対象画面について仕様JSONを生成してください。

---

# 1. Input Data

今回の変換では、以下の3つの入力データがPython実行環境から提供されます。
...
これは `system_requirements.md` から生成済みのシステム要件JSONです。
...
これは `_trace_index.md` から生成済みのTrace Index JSONです。
...
これは今回の実装対象となる1画面の画面設計Markdownです。

この3つの入力データを今回の変換に使用してください。
...
生成JSONの `source` には、以下の情報を設定してください。
...
`system_requirements` と `trace_index` は固定値です。

`screen_design` はPython実行環境から別途提供される `SCREEN_DESIGN_FILE` の値を設定してください。

`SCREEN_DESIGN_FILE` の値が提供されていない場合は、AIが推測してはいけません。
...
システム全体の要件・データ仕様・認証仕様・既存実装上の制約等を構造化したJSONです。

システム全体に関わる仕様については、このJSONを優先してください。

## TRACE_INDEX_JSON

画面とTrace IDの対応関係を構造化したJSONです。
...
複数の入力データに矛盾が存在する場合、以下の優先順位で判断してください。

1. SYSTEM_REQUIREMENTS_JSON
2. TRACE_INDEX_JSON
3. SCREEN_DESIGN_MD
4. SCREEN_DESIGN_MD内のASSUMPTION
5. AIによる推測
...
`SCREEN_DESIGN_MD` に記載された1画面のみを今回の変換対象としてください。
...
と記載されている場合、SCR-001のJSONにはSCR-002への遷移情報を記録しますが、SCR-002の仕様そのものは生成しません。

---

# 7. Requirement ID Resolution

既存の要件IDまたはTrace IDが `TRACE_INDEX_JSON` に存在する場合は、そのIDをそのまま使用してください。
...
`SCREEN_DESIGN_MD` から以下を抽出してください。
...
`SYSTEM_REQUIREMENTS_JSON` にAPI仕様が存在する場合は、その内容と画面設計の内容を照合してください。
...
特に以下については `SYSTEM_REQUIREMENTS_JSON` に記載された仕様を確認してください。
...
# 10. Trace Index Integration

`TRACE_INDEX_JSON` に対象画面のTraceが存在する場合、画面設計から抽出した要件との対応を確認してください。
...
入力データに存在しない具体的な値を仕様として断定してはいけません。
...
`manual_confirmation` は、入力データだけでは仕様を確定できない場合のみ使用してください。
...
今回生成するJSONの論理的な出力先は以下です。

```text
requirements/ai/generated/screens/{SCREEN_ID}.json
```

`{SCREEN_ID}` には `SCREEN_DESIGN_MD` の対象画面IDを使用してください。

実際のファイル保存はPython/GitHub Actions側で行います。
...
1. `SYSTEM_REQUIREMENTS_JSON` を使用したか
2. `TRACE_INDEX_JSON` を使用したか
3. `SCREEN_DESIGN_MD` を使用したか
4. `SCREEN_DESIGN_MD` の対象画面IDとJSONの `screen.id` が一致しているか
5. 入力データにない仕様を追加していないか
6. システム要件との矛盾を検出したか
7. 曖昧な仕様を推測で確定していないか
8. requirement → acceptance → test の対応が存在するか
9. `source` に実際の入力ファイル情報が記録されているか
10. JSONとして構文的に正しいか
11. JSON以外の文字列を出力していないか
12. 出力対象が `SCREEN_DESIGN_MD` の1画面だけになっているか
